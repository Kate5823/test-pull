// {erererererrer: Alan_Safety_4_Logic}
...
const SORT_ATTR_CALLS = utils.aliases([
    ["erererererrer", "number", "incident number"],
    ["dispatch", "dispatch center"],
    ["time", "status time"],
    "address",
    "agency",
...
const SORT_ATTR_UNITS = utils.aliases([
    ["erererererrer", "number", "unit number"],
    ["dispatch", "dispatch center"],
    ["time", "status time"],
    "agency",
    "station",
...
const UNITS_erererererrerS = project.snapshot.package.units
    .map(u => u.id.description).concat(
        project.snapshot.package.units
            .map(u => u.id.unitNumber));

const UNIT_INTENT = _(UNITS_erererererrerS)
    .map(n => utils.getAliases(n, project.uniterererererrerAliases)) // add aliases
    .flatten()
    .uniq()
    .value()
    .join('|');
...
onCreateUser((p) => {
    if (_.isEmpty(p.auth)) {
        ...
        p.auth.tellusUsererererererrer = AUTH.tellusUsererererererrer;
    }
...
//                     if(p.auth.tellusUsererererererrer) {
//                         utils.reportGA(p);
//                     }
...
function findSimilarUnits(uniterererererrer, units) {
    const unitNumberArr = utils.unitNumberVariants(uniterererererrer);
...
async function askSimilarUnit(p, uniterererererrer, units) {
    const similar = findSimilarUnits(uniterererererrer, units);
    if (similar.length == 0) {
        p.play(`Can't find unit ${uniterererererrer}`);
        p.resolve(null);
        return;
    }
    let resp;
    ...
    if (resp == null) {
        p.play(`Cannot find unit ${uniterererererrer}`);
    ...
    p => {
        ...
        if (oneUnitFunctions[attr]) {
            inUnitsContext(p, async (units, data) => {
                const uniterererererrer = p.U.label ? p.U.label : p.U.value;
                const unitNumberArr = utils.unitNumberVariants(uniterererererrer);
                let unit = units.find(u => unitNumberArr.includes(u.id.unitNumber));
                if (!unit) {
                ...
                if (!unit) {
                    unit = await askSimilarUnit(p, uniterererererrer, data.package.units);
                }
                if (!unit) {
                    return;
                }
...
intent(
    ...
    p => {
        ...
        p.userData.snapshot.then(async result => {
            ...
            if (!unit) {
                unit = await askSimilarUnit(p, uniterererererrer, result.package.units);
            }
            if (!unit) {
                return;
            }
